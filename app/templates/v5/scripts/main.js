            ...
            function apiSuccess(data, brandID, brandName) {
                ...
                if (as_count > 0) {
                  ...
                  if (as_count > 0) {
                    ...
                    if (as_count > 9) {
                      $('.autosuggest.multi-brand div#brand-' + brandID + ' ul div').append('<a class="btn btn-submit" target="_blank" href="//' + document.location.host.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?b_id=' + brandID + '&q=' + search_query + '&displayMode=BrandOnly">View All</a>');
                    }
  ...
  $('#searchInput').on('keyup paste', function () {
    if ($('#searchInput').val().length > 3 && $('#searchInput').val().length <= 250) {
      ...
      window.timer = setTimeout(function () {
        ...
        position.top = position.top + $('#searchInput').outerHeight() - 2;
        var searchWidth = $('#searchInput').outerWidth();
    ...
    success: function success(data) {
      ...
      function apiSuccess(data) {
        ...
        if (as_count > 0) {
          ...
          if (as_count > 9) {
            $('#SearchAutoSuggest').append(' ' + '<div class="col-md-12" id="brand-results">' + '<div class="row"><h5 class="subheader"><span>' + as_count + ' + </span>' + resultsFound + ' in Site Name</h5></div><div class="row"><ul class="unstyled"></ul></div>');
          } else {
            $('#SearchAutoSuggest').append('<div class="col-md-12" id="brand-results">' + '<div class="row"><h5 class="subheader"><span>' + as_count + ' </span>' + resultsFound + ' in Site Name</h5></div><div class="row"><ul class="unstyled"></ul></div>');
          }
      ...
      success: function success(data) {
        ...
        function apiSuccess(data, brandID, brandName) {
          ...
          if (as_count > 0) {
            ...
            $('#searchInput').addClass('on');
            if (as_count > 9) {
              $('#SearchAutoSuggest').append('<div class="col-md-12" id="brand-'+brandID+'">' + '<div class="row"><h5 class="subheader"><span>' + as_count + ' + </span>' + resultsFound + ' in Site Name</h5></div><div class="row"><ul class="unstyled"></ul></div>');
            } else {
              $('#SearchAutoSuggest').append('<div class="col-md-12" id="brand-'+brandID+'">' + '<div class="row"><h5 class="subheader"><span>' + as_count + ' </span>' + resultsFound + ' in Site Name</h5></div><div class="row"><ul class="unstyled"></ul></div>');
            }
            if (as_count > 0) {
